...
target_joints = [0, 0, 0, 0, -np.pi/3, 0, 0, 0]
...
# print(goal_eef_pose)
print(init_eef_pose, goal_eef_pose)
...
for step in range(1, n_step+1):
    ...
    err_pose = k_utils.calc_pose_error(tar_T, cur_T, float(1e-6))
    # print(err_pose)
    dX = err_pose/delta_t

    J = jac.calc_jacobian(robot.desired_frames, cur_fk, dof)
    dq = lamb * np.dot(np.linalg.pinv(J), dX)
    # print(current_joints, dq)
    current_joints = np.array([(current_joints[i] + dq[i]*delta_t) for i in range(dof)]).reshape(7,)
    joints.append(np.hstack((np.zeros(1),current_joints)))
