...
class PickAction(ActivityBase):
    ...
    def get_possible_transitions(self, scene:Scene=None, action:dict={}):        
        ...
        for grasp_pose in action[self.action_info.GRASP_POSES]:
            ...
            # for idx, objects in enumerate(next_scene.logical_states.get(supporting_obj.name).get(next_scene.state.support)):
            #     if next_scene.objs[pick_obj].name == objects.name:
            #         next_scene.logical_states.get(supporting_obj.name).get(next_scene.state.support).pop(idx)
            ...
            self.scene_mngr.attach_object_on_gripper(pick_obj, True)
