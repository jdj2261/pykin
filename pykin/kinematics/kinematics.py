...
class Kinematics:
    ...
    def analytical_inverse_kinematics(self, pose):
        ...
        # T_Wo_BL = np.array([[np.sqrt(2)/2, np.sqrt(2)/2, 0, L_Wo_B],
        #                     [-np.sqrt(2)/2, np.sqrt(2)/2, 0, -h_Wo_B],
        #                     [0, 0, 1, H_Wo_B],
        #                     [0, 0, 0, 1]])

        # T_Wo_BR = np.array([[-np.sqrt(2)/2, np.sqrt(2)/2, 0, -L_Wo_B],
        #                     [-np.sqrt(2)/2, -np.sqrt(2)/2, 0, -h_Wo_B],
        #                     [0, 0, 1, H_Wo_B],
        #                     [0, 0, 0, 1]])
        ...
        # T_0_6 = np.dot(np.dot(np.matrix.transpose(np.dot(T_Wo_BL, T_BL_0)),
        #                T_Wo_Goal),
        #                np.matrix.transpose(TM_6_GR))
    ...
    def numerical_inverse_kinematics(self, current_joints, target, desired_tree=None):
        ...
        def calc_pose_error(T_ref, T_cur):
            ...
            rot_err = np.dot(np.linalg.inv(T_cur[:3, :3]), T_ref[:3, :3])
